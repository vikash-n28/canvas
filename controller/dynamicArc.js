angular.module("myApp").controller('', function($scope) {
  ...
  function getDistance(center,point1,point2) {
    ...
    //  var r3xV = point1.x - point2.x;
    //  var r3yV = point1.y - point2.y;
    //  var numerator = point2.y*(center.x-point1.x)+center.y*(point1.x-point2.x)+point1.y*(point2.x-center.x);
    //  var denominator = (point2.x-center.x)*(center.x-point1.x) + (point2.y-center.y)*(center.y-point1.y);
    //  var ratio = numerator/denominator;
    //  var angleRad = Math.Atan(numerator,denominator);
    //  console.log(angleRad);
    // var thetaPo = Math.abs(2*Math.atan2(point1.y - point2.y, point1.x - point2.x)*180/Math.PI);
    // console.log(thetaPo);
    // var step = 2*Math.PI/70;
    // for(var i=0; i<thetaPo; i++){
    //   var cp1 = center.x + r1*Math.cos(i);
    //   var cp2 = center.y - r1*Math.sin(i);
    // }
    ...
    // console.log("distance", r3);
     ...
     for (var i = 0; i < segs; i++) {
       ...
       var d1 = calDistance(point1, {x:endX,y:endY});
       var d2 = calDistance(point1, {x:cpX1,y:cpY1});
       if ((d1 < 1) || (d2 < 1)) {
           startAngle = angle;
           break;
       }
     ...
     for (var i = 0; i < segs; i++) {
         ...
         var d1 = calDistance({x: x3,y: y3}, {x:endX,y:endY});
         var d2 = calDistance({x: x3,y: y3}, {x:cpX1,y:cpY1});
         if ((d1 < 2) || (d2 < 2)) {
             angle += theta;
